...
@section('title', 'Reports - Smart Waste Management')
@section('page-title', 'Waste Reports')
@section('page-description', 'Monitor and manage all waste collection reports with AI-powered insights')
    ...
    <div class="col-lg-8">
        <div class="card h-100">
            <div class="card-header text-white d-flex justify-content-between align-items-center" style="background-color: var(--primary-color); border-bottom: 1px solid var(--gray-200);">
                ...
                <span class="badge bg-white text-primary px-3 py-2 fw-semibold" style="color: var(--primary-color) !important; border: 1px solid var(--primary-color);">{{ $reports->total() }} Total</span>
            </div>
            <div class="card-body p-0">
                @if($reports->count() > 0)
                    <div class="reports-container">
                        @foreach($reports as $index => $report)
                        <div class="report-card p-4 border-bottom" data-aos="fade-up" data-aos-delay="{{ $index * 50 }}">
                            <div class="row align-items-center">
                                    ...
                                    <div class="d-flex align-items-start">
                                        ...
                                        <div class="flex-grow-1">
                                            <div class="d-flex align-items-center mb-2">
                                                <h6 class="mb-0 fw-bold text-primary">#{{ $report->id }}</h6>
                                                <span class="mx-2 text-muted">•</span>
                                                <span class="text-muted">{{ $report->user->name ?? 'Unknown User' }}</span>
                                            </div>
                                            <div class="mb-2">
                                                @if($report->waste_type)
                                                    <span class="badge bg-{{ $report->waste_type === 'organic' ? 'success' : ($report->waste_type === 'plastic' ? 'info' : ($report->waste_type === 'metal' ? 'secondary' : ($report->waste_type === 'glass' ? 'primary' : 'danger'))) }} me-2">
                                                        <i class="fas fa-{{ $report->waste_type === 'organic' ? 'leaf' : ($report->waste_type === 'plastic' ? 'bottle-water' : ($report->waste_type === 'metal' ? 'cog' : ($report->waste_type === 'glass' ? 'wine-glass' : 'exclamation-triangle'))) }} me-1"></i>
                                                ...
                                                @endif
                                                <span class="badge bg-{{ $report->status === 'pending' ? 'warning' : ($report->status === 'processing' ? 'info' : ($report->status === 'processed' ? 'success' : 'secondary')) }}">
                                ...
                                <div class="col-md-4 text-end">
                                    <div class="mb-2">
                                        <small class="text-muted d-block">{{ $report->created_at->format('M d, Y') }}</small>
                                        <small class="text-muted">{{ $report->created_at->format('H:i A') }}</small>
                                    </div>
                                    <div class="btn-group" role="group">
                                        <input type="checkbox" class="form-check-input me-2 report-checkbox" value="{{ $report->id }}" onchange="toggleBulkActions()">
                                        <button class="btn btn-outline-primary btn-sm" onclick="viewReport({{ $report->id }})" data-bs-toggle="tooltip" title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        @if($report->image_path)
                                            <a href="{{ Storage::url($report->image_path) }}" target="_blank" class="btn btn-outline-info btn-sm" data-bs-toggle="tooltip" title="View Image">
                                                <i class="fas fa-image"></i>
                                            </a>
                                        @endif
                                        <button class="btn btn-outline-success btn-sm" onclick="editReport({{ $report->id }})" data-bs-toggle="tooltip" title="Edit Report">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button class="btn btn-outline-warning btn-sm" onclick="updateStatus({{ $report->id }})" data-bs-toggle="tooltip" title="Update Status">
                                            <i class="fas fa-tasks"></i>
                                        </button>
                                        <button class="btn btn-outline-danger btn-sm" onclick="deleteReport({{ $report->id }})" data-bs-toggle="tooltip" title="Delete Report">
            ...
            <div class="card-body p-4">
                <div class="stats-item mb-3 p-3" style="background-color: var(--gray-100); border: 1px solid var(--gray-200);">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-warning px-3 py-2">{{ $reports->where('status', 'pending')->count() }}</span>
                    </div>
                </div>
                <div class="stats-item mb-3 p-3" style="background-color: var(--gray-100); border: 1px solid var(--gray-200);">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-info px-3 py-2">{{ $reports->where('status', 'processing')->count() }}</span>
                    </div>
                </div>
                <div class="stats-item mb-3 p-3" style="background-color: var(--gray-100); border: 1px solid var(--gray-200);">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-success px-3 py-2">{{ $reports->where('status', 'processed')->count() }}</span>
                    </div>
                </div>
                <div class="stats-item p-3" style="background-color: var(--gray-100); border: 1px solid var(--gray-200);">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-primary px-3 py-2">{{ $reports->where('status', 'collected')->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
